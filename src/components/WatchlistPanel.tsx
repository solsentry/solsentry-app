...
import { 
  Plus, ChevronDown, ChevronRight, MoreHorizontal, ArrowUp, ArrowDown, 
  Eye, Trash2, ExternalLink, Search, Download, FolderPlus, Tag,
  Bell, CheckSquare, Square, X, Edit2, Copy
} from 'lucide-react'
...
function RiskSparkline({ data, color }: { data: number[]; color: string }) {
  ...
  return (
    <div className="rounded bg-card border border-border flex items-center justify-center" style={{ width: width + 8, height: height + 8 }}>
...
function SlotsProgressBar({ used, max }: { used: number; max: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-[10px] font-mono ${isNearLimit ? 'text-primary' : 'text-muted-foreground/60'}`}>
        {used}/{max}
      </span>
    </div>
  )
...
function EmptyState({ onAddFirst }: { onAddFirst: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
      ...
      <Button 
        onClick={onAddFirst}
        className="bg-primary hover:bg-[#d97706] text-black font-medium"
      >
...
function WatchlistRow({ 
  ...
  return (
    <div className={`border-b border-[#1F1B14] last:border-b-0 transition-colors ${isSelected ? 'bg-card/70' : ''}`}>
      {/* Main Row */}
      <div 
      ...
      >
        <div className="flex items-center gap-3">
          {selectionMode ? (
            <button onClick={(e) => { e.stopPropagation(); onSelect(); }} className="text-muted-foreground/60 hover:text-muted-foreground">
              {isSelected ? (
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground/60 hover:text-muted-foreground">
                <MoreHorizontal className="w-4 h-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="bg-background border-border">
              <DropdownMenuItem asChild>
                <Link href={`/operator/${item.wallet}`} className="text-muted-foreground hover:text-foreground">
                  <ExternalLink className="w-4 h-4 mr-2" />
                  View full profile
                </Link>
              </DropdownMenuItem>
              <DropdownMenuItem 
                onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(item.wallet); }}
              ...
              <DropdownMenuItem 
                onClick={(e) => { e.stopPropagation(); setIsEditingNotes(true); onToggle(); }}
                className="text-muted-foreground hover:text-foreground"
              >
        ...
        <div className="px-4 pb-4 pt-2 pl-12 bg-[#0d0d0d] animate-in slide-in-from-top-2 duration-200">
          ...
          {item.tags && item.tags.length > 0 && (
            <div className="flex items-center gap-1.5 mb-3">
              ...
              {item.tags.map(tag => (
                <Badge key={tag} variant="outline" className="text-[9px] px-1.5 py-0 h-4 bg-card text-muted-foreground border-border">
                  {tag}
                </Badge>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-3">
            <div>
              ...
              <span className={`text-sm font-mono ${(item.pnl24h || 0) >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
                {formatPnl(item.pnl24h || 0)}
              </span>
            </div>
            <div>
              <span className="text-[10px] text-muted-foreground/60 block mb-1">PnL 7d</span>
              <span className={`text-sm font-mono ${(item.pnl7d || 0) >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
          ...
          {item.recentTokens && item.recentTokens.length > 0 && (
            <div className="mb-3">
              <span className="text-[10px] text-muted-foreground/60 block mb-1.5">Recent Activity</span>
          ...
          {/* Risk chart larger */}
          <div className="mb-3">
            <span className="text-[10px] text-muted-foreground/60 block mb-1.5">Risk History (7d)</span>
          ...
          <div className="mb-3">
            ...
            {isEditingNotes ? (
              <div className="flex gap-2">
                ...
                />
                <Button size="sm" onClick={handleSaveNotes} className="h-8 px-2 bg-[#22c55e] hover:bg-[#16a34a] text-black">
                  Save
                </Button>
                <Button size="sm" variant="ghost" onClick={() => setIsEditingNotes(false)} className="h-8 px-2 text-muted-foreground/60">
                  <X className="w-3 h-3" />
                </Button>
              </div>
            ) : (
          ...
          <div className="flex items-center gap-4 text-[10px] text-muted-foreground/60 mb-3">
            <span>Last activity: <span className="text-muted-foreground">{item.lastActivity}</span></span>
            <span>7d rug count: <span className="text-muted-foreground">{item.rugCount7d}</span></span>
            {item.group && <span>Group: <span className="text-muted-foreground">{item.group}</span></span>}
          </div>
...
export default function WatchlistPanel() {
  ...
  const handleExportCSV = () => {
    const headers = ['Wallet', 'Alias', 'Risk Level', 'Group', 'Tags', 'Notes', 'PnL 24h', 'PnL 7d']
  ...
  const filteredAndSortedList = useMemo(() => {
    return watchlist
      .filter(item => {
        ...
        if (filter === 'high-risk') return item.riskLevel === 'CRITICAL' || item.riskLevel === 'HIGH'
        return true
      })
      .sort((a, b) => {
              ...
              <div className="flex items-center gap-2 mt-2">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" className="h-7 text-xs bg-transparent border-border text-muted-foreground hover:bg-card">
                      {filter === 'all' ? 'All' : filter === 'changed' ? 'Changed' : 'High risk'}
                      <ChevronDown className="w-3 h-3 ml-1" />
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="bg-background border-border">
                    <DropdownMenuItem onClick={() => setFilter('all')} className="text-muted-foreground">All</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setFilter('changed')} className="text-muted-foreground">Changed today</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setFilter('high-risk')} className="text-muted-foreground">High risk</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="bg-background border-border">
                    <DropdownMenuItem onClick={() => setSort('alpha')} className="text-muted-foreground">Alphabetical</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setSort('risk')} className="text-muted-foreground">Risk level</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setSort('change')} className="text-muted-foreground">Last change</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </>
