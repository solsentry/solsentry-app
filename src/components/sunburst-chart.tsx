...
import { BotCluster, ClusterWallet, RISK_COLORS, RiskLevel, SelectedItem } from '@/lib/cluster-data'
...
export function SunburstChart({ clusters, onSelect, selectedItem }: SunburstChartProps) {
  ...
  const handleDoubleClick = useCallback((cluster: BotCluster) => {
    router.push(`/cluster/${cluster.id}`)
  }, [router])

  // Check if an item is selected
  const isItemSelected = useCallback((nodeData: HierarchyNode): boolean => {
  ...
  useEffect(() => {
    ...
    const arcGroups = svg.selectAll('g.arc')
      .data(nodes)
      .join('g')
      .attr('class', 'arc')
    ...
    arcGroups.append('path')
      ...
      .on('mouseleave', function(event, d) {
        ...
        d3.select(this)
          ...
          .style('filter', selected ? `drop-shadow(0 0 8px ${RISK_COLORS[nodeData.risk || 'MEDIUM']}50)` : 'none')
      })
    ...
    glowGradient.append('stop')
      .attr('offset', '70%')
      .attr('stop-color', '#18150F')
    
    glowGradient.append('stop')
      .attr('offset', '100%')
      .attr('stop-color', '#1F1B14')
          ...
          {tooltipData.type === 'cluster' && tooltipData.cluster && (
            <>
              ...
              <div className="mt-1.5 grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px]">
                ...
                <span className="font-mono text-muted-foreground text-right">{tooltipData.cluster.walletCount}</span>
                <span className="text-muted-foreground/60">Deploys</span>
                <span className="font-mono text-muted-foreground text-right">{tooltipData.cluster.totalDeploys.toLocaleString()}</span>
                <span className="text-muted-foreground/60">Rug Rate</span>
                <span className="font-mono text-muted-foreground text-right">{tooltipData.cluster.rugRate.toFixed(1)}%</span>
              </div>
            </>
          )}
          {tooltipData.type === 'wallet' && tooltipData.wallet && (
            <>
              ...
              <div className="mt-1 grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px]">
                <span className="text-muted-foreground/60">Deploys</span>
                <span className="font-mono text-muted-foreground text-right">{tooltipData.wallet.deploys.toLocaleString()}</span>
                <span className="text-muted-foreground/60">Rugs</span>
                <span className="font-mono text-muted-foreground text-right">{tooltipData.wallet.rugs.toLocaleString()}</span>
                <span className="text-muted-foreground/60">Rug Rate</span>
                <span className="font-mono text-muted-foreground text-right">{tooltipData.wallet.rugRate.toFixed(1)}%</span>
              </div>
            </>
          )}
        </div>
