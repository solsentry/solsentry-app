...
const RichTraceNode = memo(function RichTraceNode({ data, selected }: NodeProps) {
  ...
  const borderColor = selected ? "#C17D0E" :
    node.type === "victim" ? "#ef4444" :
    node.type === "cex" ? "#22c55e" :
    (node.type === "mixer" || node.type === "bridge") ? "#a855f7" :
    (node.risk && node.risk >= 80) ? "#ef4444" :
    (node.risk && node.risk >= 50) ? "#f59e0b" : "#2A251C";
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontWeight: 700, fontSize: 11, color: accent, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      ...
      {node.tag && (
        <div style={{
          ...
          background: accent, color: "#100E0A", fontSize: 9, fontWeight: 600,
...
const RichTraceEdge = memo(function RichTraceEdge({ id, sourceX, sourceY, targetX, targetY, data, selected }: any) {
...
export function TraceNetworkGraph({
  ...
  const [currentMode, setCurrentMode] = React.useState<"full" | "money-flow" | "relationships" | "deep-trace">(mode);

  // Simple filtering based on mode
  const filteredNodes = useMemo(() => {
    if (currentMode === "money-flow") {
      return nodes.filter(n => ["victim", "split", "hop", "consolidation", "cex"].includes(n.type));
    }
  ...
  const handleNodeClick = useCallback((node: TraceWalletNode) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height, width: '100%' }}>
      ...
      <div style={{ padding: '8px 12px', background: '#1a1a1a', borderBottom: '1px solid #2A251C', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        {selectedNode && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <button onClick={() => setSelectedNode(null)} style={{ background: 'none', border: 'none', color: '#f59e0b', fontSize: 18, cursor: 'pointer', lineHeight: 1 }}>×</button>
            </div>

            <div style={{ fontFamily: 'monospace', fontSize: 12, wordBreak: 'break-all', background: '#111', padding: 8, borderRadius: 4, marginBottom: 12 }}>
            ...
            <div style={{ display: 'grid', gap: 8, marginBottom: 16 }}>
              ...
              {selectedNode.risk !== undefined && (
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <strong style={{ color: selectedNode.risk > 70 ? '#ef4444' : '#f59e0b' }}>{selectedNode.risk}</strong>
                </div>
              )}
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <a href={`/operator/${selectedNode.address}`} style={{ color: '#C17D0E', fontSize: 13, textDecoration: 'none' }}>
                  → Ver perfil completo do operador
                </a>
                <a href={`/drain/${selectedNode.address}`} style={{ color: '#C17D0E', fontSize: 13, textDecoration: 'none' }}>
            ...
            <div style={{ marginTop: 'auto', paddingTop: 12, borderTop: '1px solid #2A251C', fontSize: 11, color: '#555' }}>
              Clique em outros nós para explorar conexões. Mais dados (timeline completa, export, histórico de risco) em breve.
            </div>
          </div>
        )}
      </div>
