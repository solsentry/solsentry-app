...
function DetectionChip({ type, compact = false }: { type: DetectionType; compact?: boolean }) {
  ...
  const chips: Record<NonNullable<DetectionType>, { icon: React.ReactNode; label: string; labelShort: string; className: string }> = {
    sns: {
    ...
    <div className={cn('relative', styles.container, className)}>
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className={cn('flex flex-1 items-center px-3', styles.input.replace('px-5', '').replace('px-4', '').replace('px-3', ''))}>
            <ErrorState />
          </div>
        ) : (
          <input
        ...
        <div className="flex shrink-0 items-center gap-1.5 pr-2 sm:gap-2 sm:pr-3">
          {/* Detection chip */}
          {!isLoading && !hasError && value && <DetectionChip type={detectionType} compact={variant === 'nav'} />}
