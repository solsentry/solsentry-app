...
export function OperatorsTable({ initial }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "critical")
      arr = arr.filter((o) => (o.risk_label || "").toUpperCase() === "CRITICAL");
    else if (filter === "high")
      arr = arr.filter((o) => (o.risk_label || "").toUpperCase() === "HIGH");
    else if (filter === "serial")
      arr = arr.filter(
        (o) =>
          (o.tags || []).some(
            (t) => t.toLowerCase().includes("serial") || t.toLowerCase().includes("fast_deployer"),
          ) || (o.confirmed_rugs || 0) >= 10,
      );
        ...
        <div className="ops-chips">
          {chips.map((c) => (
            <button
              ...
              className={`ops-chip ${filter === c.key ? "is-active" : ""}`}
              type="button"
            >
              {c.label}
            </button>
          ...
          <div className="ops-table" role="table">
            ...
            {view.map((op) => (
              <Link
                ...
                className="ops-row"
                role="row"
                onMouseEnter={() => setHover(op.wallet)}
              ...
              >
                ...
                <span className="ops-wallet">
                  ...
                  {hover === op.wallet && (
                    <span className="ops-peek" role="tooltip">
                      ...
                      <span className="ops-peek-row">
                        <span>Confirmed rugs</span>
                        <strong>{fmtInt(op.confirmed_rugs)}</strong>
                      </span>
                      <span className="ops-peek-row">
                        <span>Total tokens</span>
                        <strong>{fmtInt(op.total_tokens)}</strong>
                      </span>
                      <span className="ops-peek-row">
                        <span>Rug rate</span>
                        <strong>{fmtPct(op.rug_rate_pct, 1)}</strong>
                      </span>
                    </span>
                  )}
                </span>
                ...
                <span className="ops-spark">
                  <Sparkline seed={operatorSeed(op.wallet)} />
                </span>
