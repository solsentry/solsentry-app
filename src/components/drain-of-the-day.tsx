  ...
  const handleSend = async () => {
    ...
    setTimeout(() => {
      const responses: Record<string, string> = {
        mixer:
          "Os 3 mixers identificados (7kPq...mN4x, 3jRv...pL8w, 9mTx...kQ2y) fazem parte do cluster #847, conhecido por processar drains similares. Taxa de sucesso de lavagem: 78%.",
        binance:
      ...
      let response =
        "Posso ajudar com mais detalhes sobre este drain. Pergunte sobre os mixers, o cluster de operadores, padrões de cashout, ou possibilidades de recuperação.";
      ...
      <div className="px-6 py-4 border-b border-border bg-popover">
        ...
        <p className="mt-2 text-muted-foreground text-sm">
          ...
          <span className="text-muted-foreground/60">
            {" "}
            (${(data.totalUsd / 1000000).toFixed(1)}M)
          </span>{" "}
          drained from <span className="font-mono text-[#ef4444]">{data.victimAddress}</span> to{" "}
          <span className="text-[#22c55e]">{data.destinationCex}</span> via{" "}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-muted-foreground text-sm leading-relaxed">
              <span className="text-foreground font-medium">Sena AI:</span> Funds split across 3
              mixer wallets, consolidated through 2 known fast-deployer wallets{" "}
              <span className="font-mono text-muted-foreground/60">(cluster #847)</span>, terminated
              at Binance hot wallet in {data.duration}. Pattern matches{" "}
              <span className="text-primary">&apos;fast-cashout&apos;</span> template{" "}
              <span className="text-muted-foreground/60">(8 similar events 7d)</span>.
            </p>
          </div>
