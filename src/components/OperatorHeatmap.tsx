...
export default function OperatorHeatmap() {
  ...
  const stats = useMemo(
    () => ({
      ...
      avgRugRate: (operators.reduce((sum, op) => sum + op.rugRate, 0) / operators.length).toFixed(
        1,
      ),
    }),
    [operators],
  ...
  const maxHourlyDeploys = useMemo(() => {
    return Math.max(
      ...operators.flatMap((op) => op.days.flatMap((d) => d.hours.map((h) => h.deploys))),
    );
  }, [operators]);
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          <div className="bg-popover border border-border rounded-lg p-4">
            <div className="text-[11px] font-mono text-muted-foreground/60 uppercase tracking-wider mb-1">
              Tracked
            </div>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Risk Indicator */}
                  <div
                    className={`w-1.5 h-10 rounded-full ${RISK_COLORS[operator.riskLevel].bg} ${RISK_COLORS[operator.riskLevel].glow}`}
                  />
                  ...
                  <div className="w-44 shrink-0">
                    ...
                    <div className="flex items-center gap-3 text-[11px] font-mono text-muted-foreground/60">
                      ...
                      <span className={RISK_COLORS[operator.riskLevel].text}>
                        {operator.rugRate}%
                      </span>
                  ...
                  <div className="flex-1 flex items-center gap-1">
                    {operator.days.map((day, dayIdx) => {
                      ...
                      const isHovered =
                        hoveredCell?.wallet === operator.wallet &&
                        hoveredCell?.day === dayIdx &&
                        hoveredCell?.hour === undefined;
                      ...
                      return (
                        <div
                          ...
                          onMouseEnter={() =>
                            setHoveredCell({ wallet: operator.wallet, day: dayIdx })
                          }
                                    ...
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 z-20 pointer-events-none">
                                      <div className="bg-background border border-border rounded px-2 py-1 text-[10px] whitespace-nowrap shadow-lg">
                                        <div className="font-mono text-muted-foreground">
                                          {String(hourIdx).padStart(2, "0")}:00 - {hour.deploys}d /{" "}
                                          {hour.rugs}r
                                        </div>
                                      </div>
                                    </div>
                    ...
                    <div className="flex items-center gap-6 mt-4 pt-4 border-t border-border">
                      <div className="text-[11px] font-mono">
                        ...
                        <span className="text-muted-foreground">
                          {
                            DAY_LABELS[
                              operator.days.reduce(
                                (max, d, i, arr) => (d.deploys > arr[max].deploys ? i : max),
                                0,
                              )
                            ]
                          }{" "}
                          @ {String(operator.days[0].peakHour).padStart(2, "0")}:00
                        </span>
                      </div>
                      <div className="text-[11px] font-mono">
                        <span className="text-muted-foreground/60">Weekly Total: </span>
                        <span className="text-muted-foreground">
                          {operator.days.reduce((s, d) => s + d.deploys, 0)} deploys
                        </span>
                      </div>
                      <div className="text-[11px] font-mono">
                        <span className="text-muted-foreground/60">Pattern: </span>
                        <span
                          className={operator.isBotFarm ? "text-red-400" : "text-muted-foreground"}
                        >
