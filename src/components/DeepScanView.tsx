    ...
    async function load() {
      try {
        const [h, m] = await Promise.all([
          fetchHolders(mint),
          fetch(`/api/birdeye/token?mint=${mint}`).then(res => res.json()).catch(() => null)
        ]);
        setData(h);
        setMarket(m);
      } catch (e) {
    ...
    return (
      <div style={{ 
        ...
        <div style={{ animation: "pulse 1.5s ease-in-out infinite", fontWeight: 600 }}>
          {pt ? "Processando Deep Scan (Mercado & Holders)..." : "Processing Deep Scan (Market & Holders)..."}
        </div>
        <style>{`
    ...
    <div style={{ marginTop: 16, borderTop: "1px solid var(--border)", paddingTop: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        <div style={{ background: "var(--surface-2)", borderRadius: 6, padding: "10px 12px" }}>
          <div style={{ fontSize: 11, color: "var(--fg-3)" }}>{pt ? "MARKET CAP" : "MARKET CAP"}</div>
          <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "var(--font-display)" }}>
            {formatMoney(mcap)}
          </div>
        </div>
        <div style={{ background: "var(--surface-2)", borderRadius: 6, padding: "10px 12px" }}>
          <div style={{ fontSize: 11, color: "var(--fg-3)" }}>{pt ? "LIQUIDEZ" : "LIQUIDITY"}</div>
          <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "var(--font-display)", color: liquidity < 5000 ? "var(--status-critical)" : "inherit" }}>
        ...
        <div style={{ background: "var(--surface-2)", borderRadius: 6, padding: "10px 12px" }}>
          <div style={{ fontSize: 11, color: "var(--fg-3)" }}>{pt ? "TOTAL HOLDERS" : "TOTAL HOLDERS"}</div>
          <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "var(--font-display)" }}>
            {data ? fmtInt(data.count) : "--"}
          </div>
        </div>
      </div>
      ...
      {largest.length > 0 && (
        <div style={{ background: "var(--surface-2)", borderRadius: 6, padding: "12px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: data?.top1 && data.top1 > 30 ? "var(--status-critical)" : "var(--status-success)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {largest.slice(0, 10).map((h, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, fontFamily: "var(--font-mono)" }}>
                ...
                <span style={{ fontWeight: 600, color: h.pct > 15 ? "var(--status-warning)" : "var(--fg-1)" }}>
                  {fmtPct(h.pct, 2)}
                </span>
              </div>
            ))}
